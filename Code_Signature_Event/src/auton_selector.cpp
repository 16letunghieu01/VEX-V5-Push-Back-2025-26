...
void AutonSelector::update_telemetry() {
    ...
    extern pros::Motor motorTrai;
    extern pros::Motor motorPhai;
    ...
    if (temp_label && !lv_obj_has_flag(temp_label, LV_OBJ_FLAG_HIDDEN)) {
        ...
        char buffer[100];
        if (maxTemp > 50) {
            snprintf(buffer, sizeof(buffer), "Temp: L:%.1fC R:%.1fC Up:%.1fC Dn:%.1fC MAX:%.1fC!", 
                     tempLeft, tempRight, tempAbove, tempUnder, maxTemp);
            lv_obj_set_style_text_color(temp_label, lv_color_hex(0xFF0000), 0);
        } else {
            snprintf(buffer, sizeof(buffer), "Temp: L:%.1fC R:%.1fC Up:%.1fC Dn:%.1fC", 
                     tempLeft, tempRight, tempAbove, tempUnder);
            lv_obj_set_style_text_color(temp_label, lv_color_hex(0xFFFFFF), 0);
        }
